    ...
    for label, href in NAV:
        if label in MEGA:
            ...
            for child_label, child_href, child_desc in MEGA[label]:
                ...
                if href_from(page_path, child_href) == Path(page_path).name:
                    child_cls += ' active'
...
def render_hero(page_path: str, cfg: dict) -> str:
    ...
    if page_path == 'contact.html':
        return f'''<section class="hero hero-contact" style="--hero-image:url('{cfg.get("hero_image", "")}')"><div class="container hero-contact-shell"><div class="hero-copy"><p class="eyebrow">{escape(cfg['hero_kicker'])}</p><h1>{escape(cfg['hero_title'])}</h1><p class="lead">{escape(cfg['hero_text'])}</p></div><div class="hero-badges contact-badges">{badges}</div></div></section>'''
