...
@media (max-width:900px){.container{width:min(calc(100% - 1.5rem),var(--container))}.nav-toggle{display:inline-flex}.nav{position:absolute;top:calc(100% + .75rem);right:.75rem;left:.75rem;display:none;flex-direction:column;align-items:stretch;padding:.9rem;max-height:calc(100vh - 110px);overflow:auto;border:1px solid rgba(255,255,255,.08);border-radius:18px;background:rgba(13,45,97,.98);backdrop-filter:blur(12px);box-shadow:var(--shadow-xl)}.nav.is-open{display:flex}.nav a{padding:1rem;border-radius:12px}.nav-cta{margin-left:0}.hero{padding:3.75rem 0 3.15rem}.hero-layout,.split,.section-head,.cta-strip,.cta-box,.footer-bottom{grid-template-columns:1fr;flex-direction:column;align-items:flex-start}.hero-stats,.metrics-grid,.timeline,.story-grid,.pillars,.cards.cols-4,.cards.cols-3,.cards.cols-2,.feature-services-grid,.footer-grid{grid-template-columns:1fr}.hero-subpage .hero-copy{padding:1.4rem;border-radius:22px}.hero-stats{grid-template-columns:repeat(2,minmax(0,1fr))}.section{padding:3.6rem 0}.section-nav{margin-top:-.4rem}.section-nav-panel{border-radius:16px}.section-nav-shell{display:grid;grid-auto-flow:column;grid-auto-columns:minmax(220px,80%);grid-template-columns:none;overflow-x:auto;overscroll-behavior-x:contain;scroll-snap-type:x proximity;padding:.95rem;gap:.85rem}.section-link{min-height:unset;scroll-snap-align:start;padding:.95rem}.feature-services{margin-top:-1.5rem}.feature-service-card,.card,.story,.split-card,.timeline article,.metric,.cta-strip{border-radius:18px}.news-card{flex-direction:column}.news-card img{width:100%;height:12rem}.button-row{width:100%;flex-wrap:wrap}.button-row .button{flex:1 1 220px}}
