...
.news-section{background:#fff}.news-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:2rem}.news-card{display:flex;gap:1.25rem;align-items:flex-start}.news-card img{width:12rem;height:9rem;object-fit:cover;border-radius:14px;box-shadow:0 8px 20px rgba(0,0,0,.08)}.news-card-copy h3{margin-bottom:.45rem;color:var(--navy)}.news-card-copy a{color:var(--orange);font-weight:800;text-decoration:none}
...
def render_section_nav(page_path: str, section: str) -> str:
    ...
    return f'<section class="section-nav" aria-label="{escape(nav_label)}"><div class="container"><div class="section-nav-panel"><div class="section-nav-heading">{escape(heading)}</div><div class="section-nav-shell">{links}</div></div></div></section>'
...
<html lang="en">
  ...
  <body>
    ...
    <main>
      ...
      <section class="hero{' hero-home' if path == 'index.html' else ''}"{f' style="--hero-image:url(\'{cfg.get("hero_image", "")}\')"' if path == 'index.html' and cfg.get('hero_image') else ''}>
